    ...
    <div className="flex items-center justify-between py-2.5 gap-3">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          {value && <p className="text-xs font-mono text-inkSoft">{value} {unit || ""}</p>}
        </div>
      </div>
    ...
    <div className="border border-mist rounded-2xl bg-paper p-5 space-y-5">
      ...
      {attention.length > 0 ? (
        <div className="space-y-1">
          <h3 className="font-mono text-xs uppercase tracking-widest text-inkSoft">Worth a second look</h3>
        ...
        summary.insights.length > 0 && (
          <p className="text-sm text-sage-dark bg-sage-light/40 rounded-lg px-3 py-2">
      ...
      {normal.length > 0 && (
        <div>
          ...
          {showAll && (
            <div className="divide-y divide-mist/60 mt-2">
              {normal.map((i, idx) => (
                <InsightRow key={idx} name={i.name} value={i.value} unit={i.unit} status={i.status} />
              ))}
            </div>
